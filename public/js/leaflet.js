document.addEventListener("DOMContentLoaded", () => {
    ...
    stores.forEach((store) => {
      ...
      const marker = L.marker([location.coordinates[1], location.coordinates[0]]).addTo(map); // Lat, Lng
    ...
    const bounds = stores.map((store) => [store.location.coordinates[1], store.location.coordinates[0]]);
    if (bounds.length > 0) {
